...
# LOAD MODEL & DICTS
# ======================
with open("a/token_to_id_train.pkl", "rb") as f:
    token_to_id = pickle.load(f)
with open("a/id_to_token_train.pkl", "rb") as f:
...
state = torch.load("cvae_lstm_fulltrained_20260126_235953.pt", map_location=device)
...
print(f"[ℹ️] {len(midi_files)} archivos MIDI encontrados en {input_folder}")
    ...
    try:
        ...
        with torch.no_grad():
            ...
            for _ in range(N):
                ...
                tokens = [id_to_token[t.item()] for t in out if id_to_token[t.item()] not in ["<sos>", "<eos>", "<pad>"]]
        ...
        output_path = os.path.join(
            output_folder,
            f"{os.path.splitext(archivo)[0]}_CVAE_seed{seed}_sigma{sigma}_N{N}_{date}.mid"
        )
